...
#define RHO(i, j)    (*(rho + (i)*(M+1) + (j)))    // (i, j)のrho


double *phi, *Es,*rho;
double a, b, c, lin, lout, t, r, z, h, Er, Ez, Emax, Vmax;
double phi_max, error_max, error, phi_prev, error_limit,rho_prev,I,L;
int    *flag;
int    i, j, loop;
...
main()
{
  ...
  // 変数の設定。
  a = 9.8833e-2; // 外側電極の半径[m]。a < RMAXとすること
  b = 0.294e-3; // 内側電極の半径[m]。b < aとすること
  t = 2e-3; // 外側電極の厚さ[m]
  Vmax = 25e3; // 内側電極(高電圧電極)の電位[V]
  I = 0.14e-3;//[A]
  L = 45e-2;//[m]
  ...
  //for (loop = 0; loop < 3; loop++) {
	...
	// 前回ループの PHI(i, j) を phi_prev にいれておく
	        phi_prev = PHI(i, j);
	// 前回ループの RHO(i, j) を phi_prev にいれておく
	        rho_prev = RHO(i,j);
	...
	// r中心軸上のノイマン境界について PHI(i, j) を計算する
	            ...
	            PHI(i, j) = 1/6.*(PHI(i, j+1) + PHI(i, j-1) + 4*PHI(i+1, j)+ h*h*RHO(i,j)/eps0);

	// r方向とz方向のノイマン条件の格子点の例外処理。
	        else if (FLAG(i, j) == 5)
	            PHI(i, j) = 1/6.*(4*PHI(i+1, j) + 2*PHI(i, j-1)+ h*h*RHO(i,j)/eps0);
  ...
  // 電位PHI(i, j)から電界強度ES(i, j)を差分法で計算。
    ...
    for (i = 0; i <= N; i++){
        for (j = 0; j <= M; j++){
             if( b < i*h && i*h < a){
                    RHO(i,j) = I /(2 * M_PI * (i*h) * L * myu * ES(i,j));
                } else {
                     RHO(i,j) = 0;
        }
    }
