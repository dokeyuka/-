...
// パラメータの定義
#define N	500	// r方向の格子数（格子点はN + 1個）。
#define M	700	// z方向の格子数（格子点はM + 1個）（NとMは2以上にすること)
#define RMAX	0.05	// r座標の最大値[m]。ZMAX = RMAX/N * M
#define PHI0	Vmax*0.1	// 全格子点の電位の初期値 (0, Vmax, 0.3*Vmaxなど)
#define ERR_LIM	1e-4	// 収束判定誤差
...
main()
{
  ...
  for (loop = 0; error_max > error_limit; loop++) {
	...
	// r中心軸上のノイマン境界について PHI(i, j) を計算する
	  ...
	  PHI(i, j) = 1/6.*(PHI(i, j+1) + PHI(i, j-1) + 4*PHI(i+1, j));

	// r方向とz方向のノイマン条件の格子点の例外処理。
	else if (FLAG(i, j) == 5)
	  PHI(i, j) = 1/6.*(4*PHI(i+1, j) + 2*PHI(i, j-1));
 ...
 */
double electric_field(int i, int j, int sw)
{
  // r方向電界 Er
  if (i == 0)
